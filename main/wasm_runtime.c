...
esp_err_t wasm_runtime_init(void)
{
    ...
    for (int i = 0; i < WASM_MAX_MODULES; i++) {
        ...
        /* Multi-board: try PSRAM first, fall back to internal heap for boards without PSRAM
         * (ESP32-WROOM, ESP32-DevKitC, NodeMCU-32, D1 Mini ESP32, etc.) */
#if CONFIG_SPIRAM
        s_slots[i].arena = heap_caps_malloc(WASM_ARENA_SIZE,
                                            MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (s_slots[i].arena == NULL) {
            ESP_LOGW(TAG, "PSRAM arena slot %d failed, trying internal heap", i);
            s_slots[i].arena = malloc(WASM_ARENA_SIZE);
        }
#else
        s_slots[i].arena = malloc(WASM_ARENA_SIZE);
#endif
        if (s_slots[i].arena != NULL) {
            ESP_LOGD(TAG, "WASM arena slot %d: %d KB @ %p",
                     i, WASM_ARENA_SIZE / 1024, s_slots[i].arena);
        } else {
            ESP_LOGW(TAG, "WASM arena slot %d: alloc failed", i);
        }
    }
